...
A user registers a native Cosmos Coin. Once the proposal passes (i.e is approved by governance), the ERC20 module uses a factory pattern to deploy an ERC20 token contract representation of the Cosmos Coin. Note that the native Side coin cannot be registered, as any coin including "evm" in its denomination cannot be registered.

1. User submits a `RegisterCoinProposal`
2. Validators of the Side Hub vote on the proposal using `MsgVote` and proposal passes
...
2. Validators of the Side chain vote on the proposal using `MsgVote` and proposal passes
...
- There shouldn't exist any native Cosmos Coin ERC20 Contract (eg Side, Atom,
