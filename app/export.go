...
func (app *App) prepForZeroHeightGenesis(ctx sdk.Context, jailAllowedAddrs []string) {
	...
	app.StakingKeeper.IterateValidators(ctx, func(_ int64, val stakingtypes.ValidatorI) (stop bool) {
		...
		//valAddress := sdk.ValAddress(val.GetOperator())
		//scraps, _ := app.DistrKeeper.GetValidatorOutstandingRewardsCoins(ctx, valAddress)
		// feePool := app.DistrKeeper.GetFeePool(ctx)
		// feePool.CommunityPool = feePool.CommunityPool.Add(scraps...)
		// app.DistrKeeper.SetFeePool(ctx, feePool)
		// app.DistrKeeper.FeePool = feePool

		if err := app.DistrKeeper.Hooks().AfterValidatorCreated(ctx, sdk.ValAddress(sdk.MustAccAddressFromBech32(val.GetOperator()))); err != nil {
			panic(err)
		}
		return false
	})
