...
func DefaultConfig() Config {
	...
	return Config{
		...
		ChainID:           fmt.Sprintf("sidechain_%d-1", tmrand.Int63n(9999999999999)+1),
		NumValidators:     4,
		BondDenom:         "aside",
...
// NewAppConstructor returns a new Sidechain AppConstructor
func NewAppConstructor(encodingCfg params.EncodingConfig) AppConstructor {
	return func(val Validator) servertypes.Application {
		return app.NewSidechain(
...
func New(l Logger, baseDir string, cfg Config) (*Network, error) {
	...
	for i := 0; i < cfg.NumValidators; i++ {
		...
		nodeDir := filepath.Join(network.BaseDir, nodeDirName, "sidechaind")
		clientDir := filepath.Join(network.BaseDir, nodeDirName, "sidechaincli")
