...
const (
	// MainnetChainID defines the Sidechain EIP155 chain ID for mainnet
	MainnetChainID = "sidechain_7071"
	// TestnetChainID defines the Sidechain EIP155 chain ID for testnet
	TestnetChainID = "sidechain_7070"
	// BaseDenom defines the Sidechain mainnet denomination
	BaseDenom = "aside"
)

// IsMainnet returns true if the chain-id has the Sidechain mainnet EIP155 chain prefix.
func IsMainnet(chainID string) bool {
	return strings.HasPrefix(chainID, MainnetChainID)
}

// IsTestnet returns true if the chain-id has the Sidechain testnet EIP155 chain prefix.
...
// GetSidechainAddressFromBech32 returns the sdk.Account address of given address,
// while also changing bech32 human readable prefix (HRP) to the value set on
// the global sdk.Config (eg: `sidechain`).
// The function fails if the provided bech32 address is invalid.
func GetSidechainAddressFromBech32(address string) (sdk.AccAddress, error) {
