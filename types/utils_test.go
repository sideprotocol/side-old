...
func init() {
	cfg := sdk.GetConfig()
	cfg.SetBech32PrefixForAccount("sidechain", "sidechainpub")
}
...
func TestGetSidechainAddressFromBech32(t *testing.T) {
	testCases := []struct {
		name       string
		address    string
		expAddress string
	...
	for _, tc := range testCases {
		addr, err := GetSidechainAddressFromBech32(tc.address)
		if tc.expError {
...
func TestSidechainCoinDenom(t *testing.T) {
	testCases := []struct {
		name     string
		denom    string
		expError bool
