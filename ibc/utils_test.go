...
func init() {
	cfg := sdk.GetConfig()
	cfg.SetBech32PrefixForAccount("side", "sidechainpub")
}
...
func TestGetTransferSenderRecipient(t *testing.T) {
	testCases := []struct {
		...
		},
		{
			...
			channeltypes.Packet{
				Data: transfertypes.ModuleCdc.MustMarshalJSON(
					&transfertypes.FungibleTokenPacketData{
						Sender:   "cosmos1",
						Receiver: "sidechain1x2w87cvt5mqjncav4lxy8yfreynn273xn5335v",
						Amount:   "123456",
					},
				),
			},
		...
		{
			"valid - cosmos sender, sidechain recipient",
			channeltypes.Packet{
				Data: transfertypes.ModuleCdc.MustMarshalJSON(
					&transfertypes.FungibleTokenPacketData{
						Sender:   "cosmos1qql8ag4cluz6r4dz28p3w00dnc9w8ueulg2gmc",
			...
			"sidechain1qql8ag4cluz6r4dz28p3w00dnc9w8ueuafmxps",
			"sidechain1x2w87cvt5mqjncav4lxy8yfreynn273xn5335v",
			false,
		},
		{
			"valid - sidechain sender, cosmos recipient",
			channeltypes.Packet{
				Data: transfertypes.ModuleCdc.MustMarshalJSON(
					&transfertypes.FungibleTokenPacketData{
						Sender:   "sidechain1x2w87cvt5mqjncav4lxy8yfreynn273xn5335v",
		...
		{
			"valid - osmosis sender, sidechain recipient",
			channeltypes.Packet{
				Data: transfertypes.ModuleCdc.MustMarshalJSON(
					&transfertypes.FungibleTokenPacketData{
...
func TestGetReceivedCoin(t *testing.T) {
	...
	testCases := []struct {
		...
		},
		{
			...
			"transfer",
			"channel-0",
			"transfer/channel-0/aside",
			"10",
			sdk.Coin{Denom: "aside", Amount: sdk.NewInt(10)},
		},
		{
...
func TestGetSentCoin(t *testing.T) {
	testCases := []struct {
		...
		{
			"get unwrapped aside coin",
			"aside",
			"10",
			sdk.Coin{Denom: "aside", Amount: sdk.NewInt(10)},
		},
		{
			"get ibc wrapped aside coin",
			"transfer/channel-0/aside",
			"10",
			sdk.Coin{Denom: teststypes.AsidechainIbcdenom, Amount: sdk.NewInt(10)},
		},
		{
